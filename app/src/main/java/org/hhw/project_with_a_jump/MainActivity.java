...
public class MainActivity extends AppCompatActivity
{
    ...
    private static String facility; // GPS를 통해 찾은 최종 시설 정보
    ...
    public void checkAccessList(String address) {
        // 해당 시설이 없다면 알림 띄우기
        // 이 기능은 나중에 사용자가 자동으로 연결 버튼을 누르는 경우, 최초 1번만 제공됨
        if (!findAddress(address)) {
            AlertDialog.Builder builder = new AlertDialog.Builder(this);

            builder.setTitle("현재 위치에 해당하는 시설 명부가 존재하지 않습니다.");
            ...
            builder.setNeutralButton("재검색", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int id) {
                    ...
                    if (!findAddress(address)) {
                        ...
                        builder.setMessage("아래 예시와 같은 형식으로 시설명을 입력해주세요.\n줄임말 X, 띄어쓰기 필수!");
                        editText.setText("예시) 국민떡볶이 덕성여대점");
